...
let
  ...
  postgres-wrapper = writeShellScriptBin "fractalstore" ''
    ...
    if [ ! -f "$PGDATA/PG_VERSION" ]; then
      ...
      ${postgresql}/bin/pg_ctl -D "$PGDATA" -l "$PGDATA/server.log" start
      sleep 2
      ${postgresql}/bin/createdb -U "$POSTGRES_USER" "$POSTGRES_DB" || true
      ${postgresql}/bin/pg_ctl -D "$PGDATA" stop
    fi
    
    # Start PostgreSQL
    echo "Starting PostgreSQL..."
    exec ${postgresql}/bin/postgres -D "$PGDATA" -k /tmp
  '';
in
